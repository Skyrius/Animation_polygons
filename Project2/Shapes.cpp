...
point4 trianglePoints [numTrianglePoints];
point3 triangleNormals [numTrianglePoints];
...
void triang() {
	trianglePoints[0] = tVertices[0]; triangleNormals[0] = point3(0.0, 1.0, 0.0);
	trianglePoints[1] = tVertices[1]; triangleNormals[1] = point3(0.0, 1.0, 0.0);
	trianglePoints[2] = tVertices[2]; triangleNormals[2] = point3(0.0, 1.0, 0.0);
}

void generateTriangle(GLuint program, ShapeData* triangleData) {
	triang();
	triangleData->numVertices = numTrianglePoints;

	glGenVertexArrays( 1, &triangleData->vao );
    glBindVertexArray( triangleData->vao );

	setVertexAttrib(program,
                    (float*)trianglePoints,  sizeof(trianglePoints),
                    (float*)triangleNormals, sizeof(triangleNormals),
                    0, 0);
}
...
point4 pVertices[5] = {
	point4 (1.0, 0.0, 1.0, 1.0  ), //0
	point4 (1.0, 0.0, -1.0, 1.0 ), //1
	point4 (-1.0, 0.0, 1.0, 1.0 ), //2
	point4 (-1.0, 0.0, -1.0, 1.0), //3
	point4 (0.0, 1.0, 0.0, 1.0  )  //4

};

void pyr() {
	//first side
	pyramidPoints[0] = pVertices[0]; pyramidNormals[0] = point3(1.0, 1.0, 0.0);
	pyramidPoints[1] = pVertices[1]; pyramidNormals[1] = point3(1.0, 1.0, 0.0);
	pyramidPoints[2] = pVertices[4]; pyramidNormals[2] = point3(1.0, 1.0, 0.0);

	//second side
	pyramidPoints[3] = pVertices[0]; pyramidNormals[3] = point3(0.0, 1.0, 1.0);
	pyramidPoints[4] = pVertices[2]; pyramidNormals[4] = point3(0.0, 1.0, 1.0);
	pyramidPoints[5] = pVertices[4]; pyramidNormals[5] = point3(0.0, 1.0, 1.0);

	//third side
	pyramidPoints[6] = pVertices[2]; pyramidNormals[6] = point3(-1.0, 0.0, 1.0);
	pyramidPoints[7] = pVertices[3]; pyramidNormals[7] = point3(-1.0, 0.0, 1.0);
	pyramidPoints[8] = pVertices[4]; pyramidNormals[8] = point3(-1.0, 0.0, 1.0);

	//fourth side
	pyramidPoints[9] = pVertices[3]; pyramidNormals[9] = point3(0.0, -1.0, 1.0);
	pyramidPoints[10] = pVertices[1]; pyramidNormals[10] = point3(0.0, -1.0, 1.0);
	pyramidPoints[11] = pVertices[4]; pyramidNormals[11] = point3(0.0, -1.0, 1.0);

	//bottom
	pyramidPoints[12] = pVertices[0]; pyramidNormals[12] = point3(0.0, -1.0, 0.0);
	pyramidPoints[13] = pVertices[1]; pyramidNormals[13] = point3(0.0, -1.0, 0.0);
	pyramidPoints[14] = pVertices[3]; pyramidNormals[14] = point3(0.0, -1.0, 0.0);

	pyramidPoints[15] = pVertices[0]; pyramidNormals[15] = point3(0.0, -1.0, 0.0);
	pyramidPoints[16] = pVertices[2]; pyramidNormals[16] = point3(0.0, -1.0, 0.0);
	pyramidPoints[17] = pVertices[3]; pyramidNormals[17] = point3(0.0, -1.0, 0.0);
}
